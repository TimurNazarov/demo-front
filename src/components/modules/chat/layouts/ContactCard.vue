  ...
  <div :class="{'chat-module-contact': true, 'chat-module-contact-active': contact.id == active_contact.id }" @click="$emit('clicked')">
    ...
    <div class="chat-module-contact-info">
      <div class="chat-module-contact-name-info">
        <span class="chat-module-contact-name">{{ this.cut_string(contact.name, 18) }}</span>
      ...
      <div class="contact-typing" v-if="contact.typing"><span>{{ this.cut_string(contact.name, 18) }} {{ $ml.get('chat.is_typing') }}...</span><i class="fas fa-pencil-alt"></i></div>
      ...
      <p v-else class="chat-module-contact-last-message">{{ $ml.get('chat.no_messages') }}</p>
    </div>
  </div>
</template>
